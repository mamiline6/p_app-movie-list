...
if( isset($_GET["id"]) ) {
    try {
        ...
        $message = "delete this movie date?";
        $id = $row["id"];
        $title = $row["title"];
...
//         header("Location: index.php?manipulation=delete&id=".$_POST["id"]);
        exit();
    } catch(PDOException $e) {
        die("データベースエラー：".$e->getMessage());
    } finally {
    ...
    <main class="">
      ...
      <div class="flex flex-wrap justify-center";>
        ...
        <?php if(isset($_GET["id"]) || isset($_POST["id"])): ?>
          <p class="flex-none w-full py-10 font-bold text-gray text-2xl uppercase"><?php echo $message; ?></p>

          <div class="w-2/6 pr-10">
            <img src="https://api.lorem.space/image/movie?w=900&h=1320" alt="<?php echo $title; ?>" class="shadow-2xl rounded">
          </div>
          <div class="w-4/6 flex-none mx-auto text-xl text-gray">
            ...
            <form id="updateform" action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="POST">
              ...
              <div class="flex flex-col mb-3 w-full">
                <label for="exampleFormControlInput1" class="form-label inline-block mb-2 ">Title</label>
                <input type="text" disabled class="form-control block w-full mb-3 px-3 py-1.5 text-base font-normal  text-gray-700 bg-gray-300 bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" placeholder="<?php echo $title; ?>"
                  name="title" value="<?php echo $title; ?>"
                />
              </div>
              <div class="flex flex-col mb-3 w-full">
                <label for="exampleFormControlTextarea1" class="form-label inline-block mb-2">Overview</label>
              ...
              <div class="flex flex-col mb-3 w-full">
                <label for="exampleFormControlTextarea2" class="form-label inline-block mb-2">Your Memo</label>
