    ...
    <!-- <main class="flex bg-fixed bg-no-repeat bg-center bg-cover bg-black/[.60] bg-blend-darken" style="background-image: url(https://api.lorem.space/image/movie?w=900&h=1320); height: 100vh;"> -->
      ...
      <?php if(isset($_GET["id"]) || isset($_POST["id"])): ?>
          ...
          <div class="max-w-lg mx-auto text-white">
            ...
            <ul class="mt-8 backdrop-blur-sm">
              <li>・ 公開日：${20**/**/**}</li>
              <!-- <li>・ 観覧日：${20**/**/**}</li> -->
            </ul>

            <div class="btn btn-group flex flex-row space-x-6 space-x mt-10">
              <a href="delete.php?id=<?php echo $row["id"]; ?>" class="text-white bg-red-500 shadow-xl rounded px-10 py-2 font-bold uppercase flex space-between">
                Delete
              </a>
              <a href="update.php?id=<?php echo $row["id"]; ?>" class="text-white bg-blue-500 shadow-xl rounded px-10 py-2 font-bold uppercase flex space-between">
                Update
              </a>
            </div>
